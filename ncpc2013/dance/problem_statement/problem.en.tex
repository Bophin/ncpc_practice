\problemname{Dance Reconstruction}

\illustration{.4}{dance}{Photo by
	\href{http://www.flickr.com/photos/34094515@N00/5053242152}{Richard Taylor}}
...
% Marek did not want anyone to stand on one place the whole time,
%so no arrow pointed from a mark back to the same mark.
...
Given the two photos, can you help Marek reconstruct the arrows on the floor? %so that each arrow
%connects two different marks?
...
Marek's time is running out, so he is interested in any placement of arrows that could produce the
two photos.
...
from person number $i$.% Note that the $i$-th number cannot be $i$, because then the person would not
%move the whole time. If there are more solutions, you can print any one.
