/*
 * Input precision validator for NCPC 2013 problem "Cliff walk"
 * by Jaap Eldering, modified from solution.
 */
...
int main()
{
	...
	VI best(2,0);
	for(int epsdir=0; epsdir<2; epsdir++) {
		// test with +/-eps, use mm for height, hours for time:
		a = (a0+(epsdir==0 ? eps : -eps)) * 1000;
		m = (m0 * (1 + (epsdir==0 ? eps : -eps))) / 3600;
		...
		for(int y=0; y<h; y++)
			for(int x=0; x<w; x++) {
				...
				if ( reach[x][y][0]+reach[x][y][1]<=12.0 && d2>best[epsdir] ) {
//					fprintf(stderr,"found best %d at %d,%d\n",d2,x,y);
					best[epsdir] = d2;
				}
			}
	}
	...
	cout << setprecision(12) << sqsize*sqrt(best[0]) << endl;
